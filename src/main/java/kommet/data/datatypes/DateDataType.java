...
public class DateDataType extends DataType
{
	...
	public String getPostgresValue (Object value) throws KommetException
	{
		...
		{
			return "'" + MiscUtils.formatPostgresDate((Date)value) + "'";
		}
		else if (value == null)
		{
			return "null";
		...
		else if (value instanceof String)
		{
			...
			SimpleDateFormat sdf = new SimpleDateFormat(!isTimestamp ? format.substring(0, Math.min("yyyy-MM-dd".length(), ((String)value).length())) : format);
			try
			{
		...
		{
			throw new KommetException("Value of type " + value.getClass().getName() + " cannot be cast to date");
		}
	}
	...
	public String getStringValue (Object value, Locale locale) throws KommetException
	{
		...
		else if (value instanceof String)
		{
			// Try to parse the string to date. Use a substring of the format pattern, because the pattern
			// includes minutes and seconds, and the input date may be just e.g. '2013-05-24'.
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd".substring(0, Math.min("yyyy-MM-dd".length(), ((String)value).length())));
			try
			{
			...
			{
				throw new KommetException("Cannot convert value " + value + " to date");
			}
		}
	...
	public Object getJavaValue (Object value) throws KommetException
	{
		...
		else if (value instanceof String)
		{
			try
			{
				return MiscUtils.truncTimeInfo(MiscUtils.parseDateTime((String)value, true));
			}
			catch (ParseException e)
			{
